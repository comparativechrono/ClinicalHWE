...
if page == "Advanced Statistical Tests":
    ...
    try:
        ...
        if st.button("Perform Permutation Test for 2x3 Table"):
            ...
            p_value_perm, observed_stat, permuted_stats = permutation_test(table)
            
            st.write(f"### Permutation Test for 2x3 Table")
            st.write(f"P-Value: {p_value_perm:.4f}")
            
            # Plot the distribution of permuted statistics and the observed statistic
            st.write("### Permutation Test Distribution")
            fig, ax = plt.subplots()
            ax.hist(permuted_stats, bins=30, alpha=0.75, color='blue', label='Permuted Statistics')
            ax.axvline(observed_stat, color='red', linestyle='dashed', linewidth=2, label='Observed Statistic')
            ...
            permuted_stats.sort()
            quantiles = np.linspace(0.001, 0.999, len(permuted_stats))  # Adjust quantiles to avoid 0 and 1
            theoretical_quantiles = chi2.ppf(quantiles, df=2)
            ax.scatter(theoretical_quantiles, permuted_stats, alpha=0.5, label='Permuted Statistics')
            ax.plot(theoretical_quantiles, theoretical_quantiles, 'r--', label='Expected')
            ...
            st.write("""
            ### Permutation Test Explanation
            The permutation test is a non-parametric method used to determine the significance of the observed test statistic by comparing it to the distribution of test statistics generated by randomly permuting the data.
            
            In this context, we randomly shuffle the genotype counts and calculate the chi-square statistic for each permutation. By repeating this process many times, we create a distribution of chi-square statistics under the null hypothesis. The p-value is then determined by the proportion of permuted statistics that are greater than or equal to the observed statistic.
            
            This approach is particularly useful when expected counts are small, as it does not rely on large sample approximations.
            """)
    except Exception as e:
